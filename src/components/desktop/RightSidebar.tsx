  ...
  return (
    <aside className="absolute top-16 right-6 flex flex-col gap-6 z-40 w-28 items-end">
      {shortcuts.map((shortcut) => {
        // The inner UI of the icon (shared between both link types)
        const IconContent = (
          <>
        ...
        // If it's the CV, render an anchor tag to trigger a native file download
        if (shortcut.id === "cv") {
          return (
            <a 
              key={shortcut.id} 
        ...
        // Otherwise, render the standard desktop window trigger
        return (
          <div 
            key={shortcut.id} 
